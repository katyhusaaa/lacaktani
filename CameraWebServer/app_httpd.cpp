...
// GPS
// Tambahkan di bagian atas app_httpd.cpp
#include "TinyGPS++.h"
extern TinyGPSPlus gps; // Mengambil variabel gps dari file utama (.ino)

static esp_err_t gps_handler(httpd_req_t *req){
    // Jika browser meminta data JSON murni
    if (httpd_req_get_url_query_str(req, NULL, 0) == ESP_OK) {
        char json[128];
        snprintf(json, sizeof(json), "{\"lat\": %.6f, \"lng\": %.6f, \"alt\": %.2f, \"sat\": %d}", 
                 gps.location.lat(), gps.location.lng(), gps.altitude.meters(), gps.satellites.value());
        httpd_resp_set_type(req, "application/json");
        return httpd_resp_send(req, json, strlen(json));
    }

    // Tampilan awal halaman /gps yang berisi script auto-update
    const char* gps_page = R"=====(
        <html><body style='font-family:monospace; background:#000; color:#0f0; padding:20px;'>
        <div id='data'>Loading GPS...</div>
        <script>
            setInterval(() => {
                fetch('/gps?raw=1').then(r => r.text()).then(t => {
    ...
    return httpd_resp_send(req, gps_page, strlen(gps_page));
}
...
// Tambahkan di bagian paling atas app_httpd.cpp jika belum ada
#include "esp_http_server.h"
#include <TinyGPS++.h>
extern TinyGPSPlus gps; 

// Fungsi handler GPS (Taruh di app_httpd.cpp)
static esp_err_t gps_handler(httpd_req_t *req){
...
// Tambahkan strukturnya di bawah fungsi tadi
httpd_uri_t gps_uri = {
    .uri       = "/gps",
    .method    = HTTP_GET,
    .handler   = gps_handler,
...
void startCameraServer() {
  ...
  // 1. Definisikan semua URI (Daftar Belanja)
  ...
  // URI tambahan lainnya (singkat saja)
  ...
  // 2. Start Server dan Daftarkan URI
  ...
  if (httpd_start(&camera_httpd, &config) == ESP_OK) {
    ...
    httpd_register_uri_handler(camera_httpd, &gps_uri); // Registrasi GPS di sini
